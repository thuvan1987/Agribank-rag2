...
# 10. Tổng kết & Đánh giá bài học

Sau khi hoàn thành Bài thực hành 3, học viên đạt được các năng lực cốt lõi:

1. **Thiết kế Pipeline xử lý dữ liệu chuẩn hóa**: Biến dữ liệu văn bản thô/HTML thành các thực thể và quan hệ có cấu trúc được kiểm soát chất lượng qua từng bước.
2. **Khai thác Gemini LLM theo kiến trúc Hybrid (Rule + LLM)**: Sử dụng Rule-based để trích xuất ứng viên (Candidates), loại bỏ bớt nhiễu trước khi gửi context cho Gemini LLM nhằm tối ưu chi phí và độ chính xác.
3. **Chuẩn hóa Thực thể (Entity Normalization)**: Xử lý triệt để các biến thể tên gọi (Alias/Canonical Name) đối với Cơ quan, Người ký, Đối tượng áp dụng và Lĩnh vực.
4. **Kiểm tra Validation & Idempotency trên Neo4j**: Áp dụng các quy tắc kiểm tra ràng buộc dữ liệu (Validation rules) và viết câu lệnh Cypher idempotent (`MERGE`, `CONSTRAINT`) đảm bảo đồ thị tri thức nhất quán khi import lại nhiều lần.

> **Ghi chú**: Đồ thị tri thức (Knowledge Graph) được xây dựng ở bài học này sẽ là nền tảng dữ liệu quan trọng cho bài học tiếp theo (Graph RAG & Tra cứu rủi ro).
